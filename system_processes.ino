void System_Processes(){
  ...
  if(enableFan==true){
    if(enableDynamicCooling==false){                                   //STATIC PWM COOLING MODE (2-PIN FAN - no need for hysteresis, temp data only refreshes after 'avgCountTS' or every 500 loop cycles)                       
      ...
      else if(temp_in>=temperatureFan){fanStatus=1;}               //Turn on fan when set fan temp reached
      else if(temp_out>=temperatureFan){fanStatus=1;}   
      else {fanStatus=0;}                //Turn off fan when set fan temp reached
      digitalWrite(FANPIN,fanStatus);                                     //Send a digital signal to the fan MOSFET
    }
